...
const DetailProgressBar = ({
  ...
  money: number;
  total: number;
  memberExpense: { item: string; amount: number }[];
  expenses: number;
}) => {
      ...
      <AnimatePresence initial={false}>
        {showDetail && (
          <motion.ul
            ...
            initial="closed"
            exit="closed"
            className="rounded-lg p-4 bg-stroke text-secondary"
          >
            {
              <motion.li layout variants={liVariants}>
                {memberExpense?.map((item, i) => (
                  <motion.div
                    key={i}
                    variants={divVariants}
                    className="flex items-center justify-between p-2 first:pt-0 last:pb-0"
                  >
                    <p className="mr-16">
                      {i + 1}.&nbsp;{item.item}
                    </p>
                    <div className="flex flex-row items-center">
                      ...
                      <motion.div
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                        onClick={() => removeExpenseItem(name, i)}
